...
func liveGetRecommend() float64 {
	res, err := utils.Get(config.ApiList.LiveRecommend)
	if err != nil {
		fmt.Println(err)
	}
...
func liveGetRoomUID(roomID string) float64 {
	res, err := utils.Get(config.ApiList.LiveGetRoomUID + "?room_id=" + roomID)
	if err != nil {
		fmt.Println(err)
	}
...
func getRoomInfoOld(uid string) float64 {
	res, err := utils.Get(config.ApiList.RoomInfoOld + "?mid=" + uid)
	if err != nil {
		fmt.Println(err)
	}
...
func getGiftBagList() []interface{} {
	res, err := utils.Get(config.ApiList.GiftBagList)
	if err != nil {
		log.Fatalln(err)
	}
...
func sendGift(roomID string, uid string) {
	...
	if len(giftBags) <= 0 {
		...
		postBody := []byte("biz_id=" + roomID +
			...
			"&platform=" + "pc" +
			"&biz_code=" + "live")
		res, err := utils.Post(config.ApiList.GiftSend, postBody)
		if err != nil {
			fmt.Println(err)
		}
		fmt.Println(res)
